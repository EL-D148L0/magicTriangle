	...
	while {_running} do {
		...
		private _newPossibleStartSides = [];
		{
			...
			private _extraPoint = _thisPSS # 1;
			{
				...
				// debug thing
				private _debugDouble = + [_line #0, _line #1, _x];
				private _debugPT = false;
				_debugDouble sort true;
				deb_msg2 append [_debugDouble];
				if ((str _debugDouble) isEqualTo (str [[3202.5,11921.3,32.6301],[3206,11923.5,33.2759],[3207.76,11920.9,33.3363]])) then {
					hint "BOING";
					_debugPT = true;
				};
				...
				if ((_sb == 0) || (_sc == 0)) then { //checking this because once i got an error zero divisor on the angle calculation //this should be rsult of the tiny altitude inaccuracy
					_allowed = false;
					continue;
				};
...
[deb_msg7, deb_msg6, deb_msg8] spawn createTrianglesToHole; 